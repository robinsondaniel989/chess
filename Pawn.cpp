...
int Pawn::move(int y, int x, int y2, int x2, vector<vector<string>> chessBoard, bool _move, bool lowerCase) {
	if (lowerCase && _move && (y - y2) < 0) {
		cout << "A pawn cannot move backwards";
	}
	else if (!lowerCase && _move && (y - y2) >= 0) {
		cout << "A pawn cannot move backwards";
	}
	else {
